...
const TimelineItem = ({ detail, index }) => {
  ...
  return (
    <div
      key={index} // aide React à tracker les items
      ref={ref}
      className={`
        relative flex items-center justify-center sm:justify-between
        mb-28 sm:mb-40 lg:mb-52 w-full
        ${isLeft ? 'flex-row' : 'flex-row-reverse'}
      `}
    >
      ...
      <motion.div
        ...
        transition={{ duration: 1.1, delay: index * 0.18, type: "spring", stiffness: 70, damping: 15 }}
        className={`
          w-full sm:w-[42%] lg:w-[40%]
          ${isLeft 
            ? 'text-left sm:text-right pr-4 sm:pr-8 lg:pr-16' 
            : 'text-left sm:text-left pl-4 sm:pl-8 lg:pl-16'}
        `}
      >
        <div className="relative inline-block">
          <motion.div
      ...
      {/* Point central + particules (animé seulement si inView) */}
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 pointer-events-none">
        <motion.div
          initial={{ scale: 1, opacity: 0.55 }}
          whileInView={{ scale: [1, 1.65, 1], opacity: [0.55, 1, 0.55] }}
          viewport={{ once: true }}
          transition={{ repeat: Infinity, duration: 4.2, ease: "easeInOut" }}
          className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gradient-to-br from-amber-400 to-amber-700 shadow-[0_0_28px_rgba(245,158,11,0.65),inset_0_0_9px_rgba(255,255,255,0.45)]"
        />
      ...
      <div className="hidden sm:block w-[42%] lg:w-[40%]" />
    </div>
  );
};
